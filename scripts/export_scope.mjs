...
// 可被逐条追问「凭什么不做」——所以权威是 JSON，代码只消费。方向与 knowledge.js→knowledge.py 一致：
// 单向生成、禁手改生成物，由 tests/scope_guard.mjs 做「权威 == 双端生成物」三方对账。
...
writeFileSync(ROOT + "frontend/functions/lib/scope_rules.js", js)
...
writeFileSync(ROOT + "backend/app/scope_rules.py", py)
