...
### H3 标点与罕见单字进了 BM25 索引（扩库当天把指标打穿地板）
- 症状：语料 55→60 条之后，`recall@5` 0.85→0.83、`mrr` 0.867→0.85、`ndcg@5` 0.789→0.773、红旗子集 0.848→0.804 四项同时跌破地板——而 5 条新内容看起来都跟这些用例无关。
- 根因（逐用例归因，不是猜的）：`tokenize` 是"逐字 + 相邻二元组"，**标点和符号也照样切**。于是「，」成为 df=54 的检索词，
  而「坐长途车回来腿肿了…」这类口语里的单字「来」（df=1）**拿到全库最高 IDF**——`肋软骨炎` 条目只因正文里有"来
  源"二字，就在肺栓塞用例上顶到第 1 名。条目越长、书面语越多，偶然撞上罕见单字的概率越高，所以**新增条目才暴露它**。
- 处置：`rag.js` 与 `rag.py` 同口径剔除 Unicode 标点/符号后再切 n-gram（无新增可调参数，不是调阈值）。
  修复后 60 条库五项指标全部**高于** 55 条时代的地板（R@5 0.860 / MRR 0.872 / nDCG@5 0.803 / 红旗 R@5 0.870）。
- 常驻判据：`tests/retriever_parity.mjs` ①把带标点查询并入 50+3 例双端比对（任一单边生效即 `0/53 一致`）；
  ②`PUNCT FILTER` 三档各断"加标点结果不变 + 改真字结果必变 + 非空召回"（正反双向，防检索器整体失灵冒充通过）。
  变异实测：旁路 JS 过滤 → `PARITY MISMATCH: 110 例` 且 `PUNCT FILTER FAIL（3 处）`，sha256 逐字节回滚。

### H4 排除性表述把"对手的病名"写进自己的条目
- 症状：为了写清鉴别诊断，条目正文常带"须先排除肺栓塞""伴消化道出血者转诊"。这类句子在**临床上是加分项，在检索里是污染**——
  它把别条目的高判别词复制进了本条目，谁问这些词谁就被顶起来。
- 根因：索引字段是 `title + keywords + text + condition` 拼成一个袋，没有"这句话是排除项"的概念。
- 处置：正文里的对手机制改用**不含对方病名/红旗词**的写法（"肺栓塞与气胸"→"肺血管与胸膜腔病因"；"消化道出血"→"出血倾向"），
  临床含义不丢。判别性描述留在 `keywords` 与 `condition` 里，它们本就是这个用途。
- 常驻判据：`fixtures/retrieval_baseline.json` 的分档地板 + `test:retrieval` 现场重算（本轮 5 例逐条归因即由该报告给出）；
  **逐用例回归锁**与**污染计数棘轮**已列台账 #71（本轮先修数据面，判据面按"新指标先量误报率再接线"的规矩排队）。


### H5 一张词表服务两种语域：改完死词，检索读数一位没动（第三十三轮实测）
- 症状：把 8 个「在 60 条语料正文里一次都不出现」的加权词逐个换成语料真写的词形（`出冷汗→出汗` 等），
  重跑 50 例银标集与 20 例留出集，**四项聚合读数与逐用例锁全部逐位不变**。看起来像"改了个寂寞"。
- 根因：加权分支的触发条件是「词命中**查询** 且 词命中语料正文」，而 `hitFlags` 只扫**原始查询串**。
  红旗表的关键词是口语侧写法（`一生中最痛`/`月经没来`/`出冷汗`），语料是指南侧写法（`出汗`/`放射痛`/`抽搐`）——
  一张手抄加权词表被同时要求满足两侧，必然一半死。上一轮把它当成"词形写错了"，只修了语料那一半。
- 处置：① 加权词表统一取**指南侧**词形（死权重棘轮由 8 收到 0，零豁免）；② 匹配面改为
  **原查询 ∪ 同义词扩展词形**，桥就是既有 `SYNONYMS` 表，不新建第三张词表；③ 把这段逻辑抽成 `queryView`，
  让 `search` 与判据共用同一份实现——判据若自己重写一遍 BM25 就是第二真值（本轮按实测否证了那种写法）。
- **为什么不能"由红旗表派生"**（本轮开局自己提的方案，被实测否证）：改成红旗表 108 词派生后
  R@5 0.86→0.85、nDCG@5 0.802806→0.799667、红旗 R@5 0.869565→0.847826，并打破逐用例锁 `ret-11`（1→0.5）；
  死词更从 8 个涨到 50 个。⇒ 两张表**用途不同、语域不同**，只能靠同义词表桥接，不能合并。台账 #83 余项据此改判关闭。
- 常驻判据：`kb_guard`（死权重零豁免 + 语料缺口名册基线 6 + 桥正反双向）；`retriever_parity` 新增
  「加权桥双端一致 + 分母自证」（桥上有词与桥不到词的样本必须同时存在，否则两端全空也冒充通过）；
  镜像面 `backend/tests/test_retriever_channels.py` 8 项原生断言（含「桥可达词集合本身零死权重」把两条判据咬合）。
- 变异实测：把 `hitFlags` 匹配面改回只扫原始 q ⇒ `kb_guard` 2 条点名红（实测够得着的加权词: 无）、
  `retriever_parity` 报 `分母失效：桥命中 0 条 / 零命中 7 条` 且 3 档比对 9 例不一致；按字节还原后全绿。
- 坑中坑：旧 M3 反向对照拿 `deadFlags[0]` 当替换对象，基线一收到 0 它就变成 `undefined`，
  于是"恰好少一个"永不可能成立——**基线为 0 时它自己就是一条必红的判据**（本轮真的撞红了才发现）。
  正解是先注入再换回（注入后 1 → 换成语料真出现的词 → 回 0），使判据在任何基线下都量"是否在语料出现"。

### H6 现有评测集测不到这条桥
- 症状：桥接通后隔离实测明确有效（`冒冷汗伴胸痛` 下 kb-001 48.05→50.05，+2.00 恰为加权值；
  不含「出汗」的 kb-053/016/057 逐位不变），但 50 例银标集与 20 例留出集读数**完全不动**。
- 根因：两套评测集的查询全部按指南侧写法出题，没有任何一条口语侧样本——评测集测不到，不等于改动无效。
- 处置：不为了"让指标好看"去动词表或阈值（R236 补注③：禁止用调参掩盖机制问题）。改为把桥的效果
  绑到**常驻行为判据**上（H5 那三条），并在 `retriever_parity` 的比对样本里补入 7 条口语查询，
  使新代码路径第一次进入双端阻断链（3 档 × 53 例 → 60 例）。
- 待办：把口语侧样本正式纳入带地板的评测集需重跑 `--write-baseline` 并重算余量，另立台账，不在本轮顺手做。


### H7 同一句话被说两遍就会漏诊——重复敏感性与"去重"这条死路（第三十四轮实测）
- 症状：把 `ret-27`（危急病例）的查询**原样重复一遍**（一个字都没换、语义完全不变），recall@5 从 0.5 掉到 **0**；
  `ret-49` 1→0.5。三档皆有：bm25 4 例（危急 2）／hybrid 2 例（危急 1）／semantic 3 例（危急 2）。
- 根因（不是巧合，是打分函数结构）：`rag` 的 BM25 是按**查询 token 逐个累加** `idf·f(k1+1)/(f+k1·…)`，
  重复即贡献翻倍；而红旗加权是**固定 `s += 2 * n`**、不随查询长度缩放 ⇒ 重复把加权的相对分量稀释掉，
  原本靠加权排进来的条目被挤下 top-5。临床场景里"家属把主诉复述一遍"极常见，这是真实暴露面。
- **看起来正确的治法已被实测否决**：查询 token 去重（`[...new Set(allQ)]`）确实把重复敏感性归零，
  但同一次跑分**打破 6 条逐用例锁**（ret-01 recall 1→0.5、ret-07/ret-13 mrr 1→0.5、ret-27 0.333→0.2、
  ret-40 0.5→0.333、ret-49 1→0.5）、聚合 MRR 跌破地板、留出集 MRR 0.636667→0.618333、nDCG@5 0.678557→0.66576。
  ⇒ 在这个打分器里**重复是带信号的**（患者强调＝该症状权重高，gold 也这么标），去重是拿检索质量换鲁棒。已按字节回滚（`cmp` 两侧全等）。
- 正解（台账 #102，未做）：把红旗加权改成**随查询词权缩放的相对量**，使"加权重"与"tf 累加"同量纲，
  重复与不重复下相对次序不变。属打分函数行为变更 ⇒ 必须单独一轮、配 31 例＋逐用例锁＋留出集重跑。
- 常驻判据：`npm run probe:register`（看守件，不进 `npm test`）——恒等复算排测量器自扰、分母自证排样本凑数、
  红旗子集零落差把"危急病例不得因写法变化而掉召回"钉成硬断言；重复敏感性按基线 4 只降不升。
  **为什么降成看守而不是直接阻断**：判据首跑就红的话，它会立刻被人当噪声关掉（常红的判据＝没有判据）；
  接线条件写在件头，读数稳定两轮且逐条归因完再并入 `npm test`。
- 同源：H6（语域落差：改指南侧词形后聚合读数不动，别把"读不出来"当成"没有效果"）、B 类（假绿）、
  R236 补注③（不可分就改机制，禁止用调参／放宽阈值掩盖机制缺陷）。
