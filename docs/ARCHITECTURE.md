...
| 红旗规则表**本体**＋载入即校验 | `rules.js` 的 `DANGER_RULES`/`COMBO_RULES`/`validateRedFlagTables`/`assertRedFlagTables` | `app/rules.py` 同名函数 | `npm run test:table` 28 项：双端整表逐字段全等（含顺序，把上一行的"探针同表"升级为"规则本体同表"）＋ 同一份变异夹具 `fixtures/red_flag_mutations.json` 在两端各施一遍、逐条必须拒且点名同一不变量 ＋ 12 类不变量全覆盖判定 ＋ advice 过裸子串红线（禁用词单一源 `fixtures/red_line_phrases.json`）；镜像端逻辑另由 `backend/tests/test_red_flag_rules.py` **原生** 27 项覆盖（subprocess 型守卫不计入 coverage.py，教训见第二十九轮） |
