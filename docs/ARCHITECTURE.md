...
| 发布工件（第十六轮补，对标 3/3 同类有 tag 触发发布工作流、我方此前全手工） | `.github/workflows/release.yml`（tag 触发） | 先跑全部门禁（ruff / type_gate / 文本卫生 / ESLint / 十五件套 / 依赖锁定 / 双端覆盖率地板 / build / bundle / openapi 漂移）**绿了才出包**：`git archive HEAD` 的 tag 源码包 + 双端 CycloneDX SBOM + `SHA256SUMS.txt`，挂到 Release 且自检"文件数 >100"防半成品。SBOM 由钉版成熟工具生成（npm 侧 cyclonedx-npm 6.x / pip 侧 cyclonedx-bom 7.4.0）并**刻意不入库**（入库即制造"陈旧副本 vs 当前 lock"的第二真值），由 `frontend/tests/sbom_guard.mjs` 校「17 个声明直接依赖连同 lock 版本逐一在单」+「组件数 ≥50」+「每组件有 purl」+「生成工具主版本相符」，四组反例实测 rc=1 |
...
| 依赖锁定对账（第十八轮补）与配置契约（第十八轮补）见 CHANGELOG v1.16.0；本轮起 `npm test` 为**十六件套**（+`test:limits`）|
