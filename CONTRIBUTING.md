...
```bash
cd frontend && npm test                       # 二十三件套，含双端契约、配置契约(env)、诊断排序金标准与知识库门禁
...
- 任何依赖变更后：`npm run lint` + `npm test` 二十三件套 + `npm run lock` + `npm run build` + `npm run test:bundle`（体积地板线）+ `npm run sbom && npm run sbom:check` 全绿。
