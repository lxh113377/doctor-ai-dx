...
第三十四轮。对标轴＝**「上一轮那句『评测集测不到桥』是错的——本轮把它证伪，并把『只换语域』这一维单独量出来」**。
**零生产行为变化**：改动的是一条新增的看守件与文档；`functions/lib/*` 与 `backend/app/*` 逐字节未变。

- **先否证自己**：第三十三轮把「改完词表后聚合读数一位没动」归因成「两套评测集的查询全部按指南侧写法出题」。
  本轮实测否证该归因：`fixtures/retrieval_cases.json` 自己的 provenance 就写着 **v2.0（ret-21..50）为患者口语化改写**；
  逐条现算 70 例 ⇒ 字面即命中加权词 14 例、**仅靠桥命中 3 例**、完全不命中 53 例。
  再做桥的开关对照（临时把匹配面改回只扫原始 q，跑完按字节还原）：**70 例指标一位没变，但 top-5 名次确有 2 例变化**
  （ret-01／ret-40，首位不变）。⇒ 真正的原因不是"没有口语样本"，而是**现有指标把 gold 差异、词面差异、语域差异混在一起动**，
  语域那一层值多少分根本读不出来。该归因错误已在台账 #99 改判。
- **新增测量面（`frontend/tests/register_probe.mjs`，`npm run probe:register`）**：配对查询由
  `retrieval_cases.json` + `SYNONYMS` **运行时现算**——把查询里某个同义词组成员换成同组另一个成员，其余一字不动，
  gold 直接继承源用例 ⇒ **零新增人工标注、不引入第二真值**，且两条查询语义等价由构造保证。
  三档实测（配对 37/50，换不出词形的不强凑）：

  | 检索档 | 语域落差 Δrecall@5 | ΔMRR | 重复敏感性（例） | 其中危急 |
  |---|---|---|---|---|
  | bm25（线上口径） | 0 | 0.018018 | 4 | **2** |
  | hybrid | 0.013513 | 0.004054 | 2 | 1 |
  | semantic | 0 | 0.002252 | 3 | **2** |

- **看守件首跑就抓到一条危急病例级缺陷**：把同一句话重复一遍（**未改任何语义**）会改变 recall@5，
  其中 `ret-27`（危急）从 0.5 掉到 **0**、`ret-49` 从 1 掉到 0.5。根因非巧合：本打分器按 token 逐个累加 idf·tf 项，
  重复即翻倍，而红旗加权是固定 `+2/词`、不跟着翻倍 ⇒ 重复会**稀释加权的相对分量**。
- **朴素治法按实测否决并回滚**：查询 token 去重确实把重复敏感性打到 0，但同一次跑分**打破 6 条逐用例锁**
  （ret-01 recall 1→0.5、ret-07/ret-13 mrr 1→0.5、ret-27 0.333→0.2、ret-40 0.5→0.333、ret-49 1→0.5）、
  聚合 MRR 跌破地板、留出集 MRR 0.636667→0.618333、nDCG@5 0.678557→0.66576
  ⇒ **重复在这个打分器里是带信号的**，去重是拿检索质量换鲁棒，已按字节还原（`cmp` 两侧全等）。
  正解记为新台账 #102：把红旗加权改成**随查询词权缩放的相对量**而非固定 +2，属打分函数行为变更、须单独一轮。
- **判据自证三向**（缺任一条本件不记通过）：分母自证（配对 ≥15，实测 37）；恒等复算（同一查询两次取数逐位相同，
  排除测量器自己动分数）；红旗子集零落差（危急配对 17 例，掉召回 0）。
  重复敏感性按「新指标先量误报率再接线」的既有规矩**降为带基线的看守棘轮**（基线 4，只准降不准升），
  不做成常红判据——常红的判据等于没有判据。接线条件已写在件内。
- **文档**：`AGENTS.md` §3 补 `npm run probe:register`（标注看守件、不进 `npm test`）；`docs/PITFALLS.md` 新增 H7
  （重复敏感性＋去重治法被实测否决的两侧读数）；`docs/ARCHITECTURE.md` 检索层补语域落差一行。
- **交付与验证**：`npm test` 二十三件套 rc=0 且四项检索读数与 v1.30.0 逐位相同（R@5 0.86／MRR 0.872333／
  nDCG@5 0.802806／红旗 R@5 0.869565）、留出集不变；`lint:js` rc=0（**首跑抓到本件一条 unused import 并修掉**）；
  后端 `selftest` 9/9；mypy 错误 0；`export_knowledge --check` 逐字节一致；`corpusSha` 与语料指纹不变。版本 1.30.0→1.31.0。
- **诚实清单**：① 本轮**没有**做成 #98（加权词由语料统计派生），因为 #99 的测量面刚建、先决条件才就位；
  ② 语域落差只有 37/50 例可配对，13 例查询不含任何同义词组成员，其"语域不变"结论对它们不适用；
  ③ 配对查询是机械换词，可能产生临床上不完全等价的表述（如把"气促"换成"喘不上气"），
  本件量的是**检索对词形的敏感度**，不是临床等价性——这点在件头注里写明，不冒充语义评测。


## [1.30.0] - 2026-09-27

第三十三轮。对标轴＝**「检索加权词表为什么修不完」——把上一轮立下的死权重棘轮从"只降不升"推到"零豁免"，并顺手否证了本轮开局自己提的方案**。
台账 #94 关闭、#83 余项以**改判**形态关闭（不是实现，是不该实现）。

- **先量再做（#83 余项改判的依据）**：07 台账把「加权词表由红旗表派生」列为下一轮首选。本轮把四档词表灌进
  **真实生成链**（改权威 JSON → 跑真 `export_knowledge` → 跑真 `retrieval_eval`）实测：
  现行 32 词 R@5 0.86／nDCG@5 0.802806／红旗 R@5 0.869565；**改由红旗表派生（108 词）后 R@5 0.85、nDCG@5 0.799667、
  红旗 R@5 0.847826，并打破逐用例锁 `ret-11`（recall@5 1→0.5）**；「活词 ∪ 规则活词」（61 词）同样判红。
  ⇒ 派生方案**不成立**并回滚：红旗表的关键词是**口语侧词形**（`一生中最痛`/`月经没来`/`说胡话`，用来匹配医生自由文本输入），
  加权词必须落在**指南侧词形**（要出现在 60 条语料正文里才计分），一张表服务两种语域，派生只会把 8 个死词变成 50 个。
- **#94 关闭（词形二分，逐词有实测）**：8 个死词逐个查语料真写的那个并替换——
  `出冷汗→出汗`、`胸痛放射→放射痛`、`高热惊厥→抽搐`、`妊娠合并→妊娠`、`果酱样便→果酱样`、
  `鞍区麻木→鞍区`、`大小便失禁→大小便`、`口唇肿胀→口唇`。替换前逐词核到具体条目（如 `果酱样` 只在 kb-034、`鞍区`/`大小便` 只在 kb-049）。
  重跑 50 例银标集 + 20 例留出集：**逐用例读数零变化、四项聚合读数与基线逐位相同、31 例全指标同 v1.29.0**。
  棘轮据此由 `DEAD_FLAG_MAX 8` 收到 **0（零豁免）**。
- **补上真正的机制洞（本轮实测才暴露）**：只改词表后 50 例读数**一位没动**，反证加权分支根本没被触发——
  `hitFlags` 此前只扫**原始查询**，而词表已改成指南侧词形，口语输入（「冒冷汗」）够不着。
  本轮把匹配面改为**原查询 ∪ 同义词扩展词形**（桥就是既有 `SYNONYMS` 表，不新增第二真值），并抽出 `queryView`
  使 `search` 与判据共用同一份实现。隔离实测：`冒冷汗伴胸痛` 下 kb-001 **48.05 → 50.05（+2.00）**、
  kb-051 33.012→35.012、kb-017 25.951→27.951，不含「出汗」的 kb-053/016/057 逐位不变——加权精确落在预测的三条上。
- **判据（28→34 项 kb_guard、+1 条双端桥比对、镜像面 +8 项原生断言）**：
  死权重零豁免、`SYNONYMS` 规范词的**语料缺口名册**（基线 6：冷汗/心悸/关节痛/牙痛/排尿困难/咽异物感，只降不升且逐条点名）、
  口语查询桥到加权词（含**反向对照**：无症状查询必须零命中，否则前两条恒真）、桥可达词集合本身零死权重（两条判据咬合）。
  变异自证：把 `hitFlags` 匹配面改回只扫原始 q ⇒ kb_guard 2 条点名红、`retriever_parity` 因分母失效（桥上有词 0／无词 7）红；还原后全绿。
  旧 M3 反向对照在基线收到 0 后自身变成**必红判据**（拿 `deadFlags[0]` 当替换对象 = `undefined`），本轮改为"先注入再换回"，
  使该判据在任何基线下都量"是否在语料出现"而不是"词表是否非空"。
- **交付链**：`retriever_parity` 的比对样本补入 7 条口语查询（3 档 × 53 例 → **3 档 × 60 例**），
  新增代码路径首次进入双端阻断链；镜像面 `test_retriever_channels.py` 24→32 项。
  全量回归：`npm test` 二十三件套 exit 0、后端 `selftest` 9/9、`coverage:js` 28 pass（rag.js 分支余量 7.1pt）、
  mypy 错误 0（阈值 0）、`export_knowledge --check` 逐字节一致、`corpusSha` 与语料指纹不变。
- **诚实清单**：① 本轮**没有**把加权词表派生自红旗表——实测会退化，已按证据否证；② 留出集 20 例四项读数在本轮改动前后
  完全相同（R@5 0.85／MRR 0.636667／nDCG@5 0.678557／红旗 R@5 0.714286），说明**现有评测集覆盖不到这条桥**，
  效果目前只由隔离实测与常驻判据背书，不由聚合指标背书；③ 6 个概念在语料里是真缺口（补条目需重建语义邻接表，
  其 ONNX 引擎不在仓内，见 `AGENTS.md` §4），本轮只把缺口名册化并设只降不升棘轮，未硬补。
