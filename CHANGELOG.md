...
第三十二轮。对标轴＝**「加一条知识要动几个文件」的下半场：把知识库本体从两端源码搬进权威 JSON**
（台账 #92，关闭 #55 的最后一块；上轮外置红旗表、本轮外置语料）。**零对外行为变化**：四组数据（60 条语料／34 组同义词／73 个症状映射／32 个检索加权词）
迁移前后与双端快照深比较逐字段全等（含列表顺序），31 例指标、检索读数、语料指纹全部逐字不变。

- **权威与生成链**：新增 `data/knowledge.json`（55,766B，由在役 `knowledge.js` 逐字段导出、非手抄，内含 `_editing` 与 `_provenance` 两段口径说明）；
  新增 `scripts/export_knowledge.mjs` 单向生成两端（`knowledge.js` 45,288→**44,341B**、`knowledge.py` 51,911→**44,011B**），
  校验与生成拆成纯函数，故带 `--check`（只比不写）与 `--selftest`（**17 种中止面**逐条真响＋生成字节可复现＋「少一条就与盘上不同」的反向对照）。
  半空止闸：`entries<50`／`synonyms<25`／`symptom_to_kb<55`／`red_flag_terms<20` 一律 rc=2 且**一个字节都不写**——防「只迁一半就发布」。
  退役 `scripts/export_kb.mjs`（旧方向 js→py；备份 `D:/global_memory/_trash/export_kb_retire_r32_20260926_2230*/export_kb.mjs`，Git 内可回滚）。
  消费方零改动：`engine/fhir/rag/retriever` 四处 import 与 `KB_BY_ID`/`kbTitleOf` 等导出形态逐字保留。
- **零行为变化的证据**：`semantic_guard` 语料指纹 `corpusSha=00704224279f` 不变；31 例评测 `0 fail`（top-1 28、top-3 30、危急 17/17、弃权 3）与 v1.28.0 逐项相同；
  检索 bm25 四项实测（R@5 0.86／MRR 0.872333／nDCG@5 0.802806／红旗 R@5 0.869565）与 `retrieval_baseline.json` 的 measured **逐字节相同**，地板与余量一位未放宽；逐用例锁 50 例全过。
- **判据**：`kb_guard` **17→28 项**——权威==JS==Py 三方全等、生成器逐字节复算、条目数一致、未声明顶层键、**死权重棘轮**，
  外加**常驻变异自证**（M1 改权威不重跑／M2 手改生成物摘一条／M3 加一个语料里不存在的加权词且此时三方数据自洽——证明棘轮不是全等判据的影子／M4 塞未声明键，另配两条反向对照防恒真）。
  `docs_link_guard` 21→23 项、`scope_guard` 46→47 项（#93：`export_scope.mjs --check` 接进守卫）、`live_smoke --selftest` 22→23 项（权威读不到必须 exit 2，不得静默放行）。
  新增镜像面原生 `backend/tests/test_knowledge_authority.py` 17 项（含「改一个字段/少一条都必须被识破」的非恒真对照），后端套件 8→9 个文件。
- **#91 按实测改判**：先枚举再决定——全仓 245 处「N 条/个/组/份/件 X」声明分属 185 族，绝大多数是 CHANGELOG 的历史叙述（不可改写），
  所以「一般化同义词族判据」不成立；真缺口是**判据看不见源码注释**。于是把知识库条数判据的射程从 `.md` 扩到 99 个源文件（生成物除外），
  历史叙述按关键词排除且**逐处计数打印**（不当作静默放行）。它首跑就抓到三处真过期：`functions/lib/data.js` 注释停在旧条数并写着已退役的生成器名、
  CONTRIBUTING/README 指向 `scripts/export_kb.mjs`（死凭据）——均已修。
- 🔴 **本轮自纠（三处自己引入的错）**：① 变异自证第一版「落盘三份文件 + fork 子进程复跑自身」，实测把 c8 对 `lib/knowledge.js` 的函数覆盖
  从 100% 扭曲成 33.33%（子进程 raw record 与主进程合并后按索引错位），全局分支虚高 81.75%→81.92%——**判据把自己量的对象改了，红和绿都不可信**，改为纯内存驱动同一批纯函数；
  ② 改写生成器时把「类型注解理由」注释挪进生成物却没重跑，`--selftest` 里那条盘上对账当场判不一致（先报错再承认，不降地板）；
...
第三十一轮。对标轴＝**「加一条红旗要动几个文件」——把红旗表本体从两端源码里搬进权威 JSON**（承接第三十轮把**校验**补上、本轮补**可改性**，台账 #89；#55 的第一块数据面）。
**零对外行为变化**：表内容与 v1.27.0 逐字段相同，31 例指标、双端契约、线上响应体均未变。

### Changed
- 🔴 **红旗表外置**：新增权威 `data/red_flag_rules.json`（13 条 DANGER + 4 条 COMBO + 否定词表 + 阳性例外词 + 血压阈值与脏读值域），
  由 `scripts/export_red_flags.mjs`（`npm run redflags:export`）单向生成 `functions/lib/red_flag_rules.js` 与 `backend/app/red_flag_rules.py`；
  `rules.js` / `app/rules.py` 从此**只留判定逻辑**。此前加一条红旗要同时改两个源码文件——双端各抄一份正是漂移的起点（#51 同族）。
  权威 JSON 由**当轮运行态代码导出**（node 直读模块），不手抄；导出前后用外置前快照做深比较，四件（两张表 + 两张词表）逐字段全等。
- `HYPERTENSION_ADVICE` 去掉 `|| "血压显著升高…需急诊处理。"` 兜底字面量（那是权威表之外的第二份建议文案，且镜像端 `next()` 本就严格）；
  改为缺规则即抛。**血压阈值与脏读值域也不再是源码字面量**，取自 `bp` 段。

### Added
- `redflags:export -- --check`：不写盘、只把"将要生成的内容"与盘上生成物逐字节比 ⇒ 抓住"改了 JSON 忘了导出"。
  双向实测：改权威 severity 后 `--check` rc=1 并点名两份生成物；还原后 rc=0。
- `red_flag_table_guard.mjs` 由 28 项增至 **37 项**：新增「权威 == JS == Py 三方全等」（两张表 + 否定词表 + 阳性例外词 + bp 段）
  + `--check` 结果核 + **文档内表条数须为派生真值**（扫 6 处「N 条 DANGER/COMBO」，实测把 CONTRIBUTING 改成 12 即点名，随后逐字节还原）。
- `backend/tests/test_red_flag_rules.py` 由 27 项增至 **32 项**：镜像端自己也核「权威 == 生成物」与「重跑 `--check` 不漂」，
  不再依赖 JS 守卫 subprocess 才看得见（第 30 轮教训的延续）。
- `CONTRIBUTING` 增两条：红旗表权威/生成物/对账链一行；**#79 落地**——"改 `functions/lib/*.js` 且 `app/*.py` 有镜像 ⇒ 同轮必须有 `backend/tests/` 原生用例，subprocess 型守卫不算"。

### Notes
- 覆盖率地板**一位未放宽**：`app/rules.py` 仍 100%、Py 全局 95.02→**95.04%**、JS 全局分支 81.73→**81.75%**；
  `lib/rules.js` 分支余量由 **1.8pt 回到 3.5pt**（表数据移出后其分支数下降，#82 的观察项当轮缓解，未动地板）。
- 迁移过程中自纠一处**新引入的文档错**：改 CONTRIBUTING 表时把镜像列写成 `backend/app/observe.js`（真实是 `observe.py`），
  下一步核对时当场发现并改回——记入项目记忆，属"文档引用须实测存在"同族（R240）。
...
### Notes
- **v1.27.0 重锚一次（已证零下游可见物）**：首推提交 `42390f2`（旧 tag 对象 `082fa91`）在 CI 被判红——
  `infra-lint` 的 `dep_completeness` 把本仓同级模块 `red_flag_dump` 当成"未声明依赖"。
  归因后修的是**判据本身**（第一方判定改由磁盘现算，不再用一张会过期的手抄名单），而不是往名单里补名字。
  重锚前逐面实测：`gh release view v1.27.0`=not found、ghcr 匿名 manifest=`404`、线上 `version` 仍 `1.26.1`，
  且旧 tag 的两条 run 均已收尾（release run 在出包前置门禁即红，未产出任何工件）⇒ 符合"零下游可见才可重锚"。
...
**红只在远端出现**。对标轴＝**「双端同逻辑」这句话里，测试这一半此前从未被对账**。
**零对外行为改动**（v1.26.0 的范围规则与 `scope_rule` 字段不变）。

### Fixed
- `app/rules.py` 镜像面覆盖率 **87.18% → 100%**（全局 94.65%，模块地板 95% 与全局地板 90% **一位未放宽**）。
  真因：第二十九轮新增的范围层（`match_scope_rule` / `validate_scope_rules` / `_occurs_unnegated` 空词守卫）
  **只有 JS 侧有原生测试**；`scope_guard.mjs` 是用 subprocess 调 Python 打印结果的，跨进程执行不计入 coverage.py，
  于是「双端各有判据」在镜像端是一句空话。
- `tests/smoke_engine.py` 里「覆盖 rules.py 的…/ 同名去重 / …五条分支」这一行注释**是不实陈述**：现存 13 条 DANGER
  与 4 条 COMBO 名字互不重叠，`_bp_crisis` 那条另有「在场即不追加」守卫，任何输入都造不出重名 ⇒ 去重的 False 分支恒不走。
  改为专项探针（临时把规则表改成重名/空关键词后必须还原）驱动该分支，并把注释里名不副实的那项摘掉。

### Added
- `backend/tests/test_scope_rules.py`（镜像面原生自证，登记进 `tests/suite.json` ⇒ selftest 由 6 套增至 **7 套**）：
  权威 `data/scope_rules.json` ↔ 生成件 `app/scope_rules.py` 逐字段全等、**12 条拒绝路径**（与 JS 侧同一批变异，
  补齐本轮此前缺的 id 非 snake_case / title 空 / doctor_note 空 / 非字符串元素）、8 正 7 反探针与 JS 侧同文本同结论、
  红旗优先于范围、`build_diagnosis` 端到端走出 out-of-scope 且 `abstain_reason` 取自权威数据、
  「人被狗咬」仍在范围内（证明边界不是画在裸单字上）。
- 空关键词兜底的双向自证：摘掉守卫后 `[""]` 会让**每个病例**都命中红旗或都判范围外（假阳性风暴／全体误弃权），
  三组变异实测各自点名对应套件 rc=1，随后按 sha256 逐字节还原。
- `docs_link_guard` 第 18/19 项：**复现命令行不得把覆盖率读数抄成第二真值**。上岗即点名 `docs/EVAL_CARD.md` 两处
  过期读数（其中「地板 88」与 fixture 里的 90 已不符，v1.13→v1.26 期间无人同步）；已删除文档侧数字、保留命令与单一源指向。
  射程刻意只取命令行——带日期的度量叙述属「当时为真」的历史陈述，首版按整行扫会误伤那两类，据实收窄。

### Notes
- **发布纪律**：v1.26.0 已让线上 `/api/health` 自报 `1.26.0`＝**有下游可见物**，故不重锚，另出补丁版本
  （另两面实测：`gh release view v1.26.0` = not found、ghcr 匿名 manifest = `404 MANIFEST_UNKNOWN`，
  即镜像与 Release 确实未产出——但线上版本号一项已足以禁止重锚）。
- 踩到并记录一个工具链坑：两次「摘掉不同守卫」的变异实验一度给出**相同的**失败行——同源同长度的
  `.pyc` 缓存（mtime 秒级 + size 相同）被复用，跑了上一轮的字节码。变异实验此后一律带 `-B`/`PYTHONDONTWRITEBYTECODE`。

## [1.26.0] - 2026-09-26

第二十九轮。对标轴＝**「引擎知道自己不知道吗」的第二半：不只是"证据不够"，还要"这件事本系统不该做"**——
台账 #76（人群/能力级适用范围）落地，并顺带把 **#55 的第一块外置临床决策数据**建起来。
**有对外行为变化**：新增 `scope_rule` 字段与三条范围规则。

### Added
- 🔴 **`data/scope_rules.json`——本仓第一块"数据即权威"的临床决策面**：三条能力级边界
  （影像与检查报告解读／给药剂量与处方方案／非人类患者），每条带 `rationale`（为什么不做）与
  `doctor_note`（该找谁），另有 `_editing` 编辑指引与 `_negation_*_rationale`。
  `scripts/export_scope.mjs`（`npm run scope:export`）单向生成 `frontend/functions/lib/scope_rules.js`
  与 `backend/app/scope_rules.py`，沿用 `knowledge.js → knowledge.py` 那条模式，不留第二真值。
  形状借鉴 `kheireddinedev00/Medico`（25,142B 单文件、4 处 `raise RuleSetError`、17 个测试专测这份数据），阈值不抄它的。
- **判定顺序**：`engine.js`↔`engine.py` 先判「该不该我做」（范围）再判「证据够不够」（阈值）；
  命中 ⇒ `abstain=true` + `scope_status=out-of-scope` + `scope_rule=<id>`，只出弃权卡、清空鉴别诊断，
  **但红旗与 evidence 照常在场**（第三条红线）。契约同步：`docs/openapi.json` 加 `scope_rule`（枚举三值+null）、
  `backend/app/models.py` 的 `DxResult` 加字段（漏则 FastAPI 丢字段）、`Dx.jsx` 显示规则标题且
  范围命中时不再谎报"证据分低于阈值"（那是另一种弃权的理由）。
- **新件套 `tests/scope_guard.mjs`（第 22 件，46 项）**：三方全等（权威==JS==Py，逐字段含顺序）、
  载入即校验的 11 类拒绝路径**全部用变异数据自证会抛**（外加"未变异原样必须通过"防校验器恒假）、
  每条规则正/反双向探针（`fixtures/scope_probes.json` 8 正 7 反）、真实链路"红旗+范围外同时在场"、
  31 例既有用例零真误伤、逐条文案含医生主导口径、双端逐探针同结论、对外文案过裸子串红线扫描。
- **线上侧**：`live_smoke` 实调断言 20→**21 项**（新增"范围命中必须带合法 `scope_rule`"，规则 id 集合取权威文件
  `data/scope_rules.json` 而非生成物；找不到权威文件时该条显式 SKIPPED，不拿空集合冒充通过）。
  离线自证矩阵仍 22 项（新断言只在真实响应路径上跑，喂假数据无意义）。

### Fixed
- 🔴 **动机是实测缺陷，不是设想**：「拍了CT／片子上说有个结节／我不懂这个报告」本轮改动前
  `top_evidence_score=59.091` **且不弃权**，首诊直接给「急性上呼吸道感染」——没有阅片能力的系统自信翻译影像结论。
  剂量类与兽医类输入此前只是**偶然**因分数低于阈值（44.147／43.747）而弃权：**理由错但结果对**，一旦阈值动就失守。
- 🔴 **新文案又撞自家裸子串红线（同族第四次）**：我给兽医规则写的 `doctor_note` 含「不替代医生判断」，
  被上一轮立的扫描抓住；本轮 additionally 发现 `engine_eval` 的"弃权理由必须含医生"**只在被用例触发的规则上生效**，
  剂量那条没被 31 例触达 ⇒ 缺医生口径的文案会静默出厂。补 `scope_guard` 逐条断言（不等用例覆盖）。
- 🔴 **抄了行为没抄探针**：`occursUnnegated` 复刻红旗层"同词先否定后阳性仍命中"的取或语义，
  但 c8 实测该分支（rules.js:87-88）**零探针覆盖**；补正/反两条探针后覆盖恢复，同时把三处
  不可达的防御性 `|| []` / `|| 4` 兜底**删掉**（生成器已保证字段形态），`lib/rules.js` 分支覆盖从
  81.72% 回到地板之上——**地板 86% 一位未降**。
- **文档行号漂移再次被上轮判据抓到**：本轮改 `engine.js`（加范围判定）使 `validateDiagnosis`/`ruleDiagnosis`/
  `answerability` 三处引用再次漂移，`docs_link_guard` 第 17 项直接点名 ⇒ 更正为 180／239／156。

### Measured
- 31 例合成病例自洽度：top-1 **28/31=90.3%**、top-3 **30/31=96.8%**、危急类 top-3 **17/17=100%**、
  弃权 3 例（**ev-25 现在因"非人类患者"这条正确理由弃权**，此前是偶然低分；gold 已声明弃权即正确 ⇒ 属改判而非误伤）、
  过度弃权 1 例（ev-09，预算不变）。检索五项与上轮同值（本轮未改语料、未改分词）。
- 二十二件套 exit0｜后端 selftest 6/6｜ruff/mypy（36 文件 0 error，新增 `scope_rules.py`）/ESLint 0｜
  `docs_link_guard` 18/0｜文本卫生 pass｜JS 覆盖率地板全过（rules.js 分支回到 ≥86%）｜E2E 7 passed（含弃权双向）。

### Notes
- 三条产品红线逻辑零改动；默认检索档仍 bm25；评测仍是同一批 31 例。
- 本轮**没有**新增任何可调阈值：范围命中是确定性关键词判定（含否定处理），`ABSTAIN_T` 仍由 probe 实算对账。
- 已知边界：规则只有三条、且只覆盖"能力级"，**人群级**（如"本系统未在该年龄段验证"）仍缺 ⇒ 与 #55 后续外置同批做。

## [1.25.2] - 2026-09-26

第二十八轮第二次收尾补丁（文档自相矛盾 + 一条新的派生判据；无产品行为改动）。

### Fixed
- 🔴 **交付包里的链路图自相矛盾**：`v1.25.1` 的 ARCHITECTURE 图已画到第⑦步（弃权），
  但同文件标题与 `README.md` 仍写「六段链路」。这是我自己上一秒改图、下一秒没回头改标题——
  也正是本轮反复在抓的那一类：**同一事实抄在多份文档里，改一处忘另一处**。
  （如实记根因：新写的"行号引用判据"是在 `v1.25.1` **打完 tag 之后**才进仓库的，
  所以发布前置门禁里根本没有这条 ⇒ 已立台账 #78「新判据必须在同轮进入出包前置门禁」。）

### Added
- **常驻判据：文档里的「N 段链路」== ARCHITECTURE 图内实际步数**（`tests/docs_link_guard.mjs` 第 19/20 项）。
  步数由图内 `①..⑦` 圈号实测去重得出，不手数也不手抄；配输入面守卫（扫到 ≥2 处「N 段」且图内 ≥5 步才算在射程内）。
  变异实测：把 README 改回「六段链路图」⇒ `FAIL … "六段链路" 应为 7 段（ARCHITECTURE 图内 ①..⑦ 实测）`，sha256 逐字节回滚。

### Notes
- 三条产品红线逻辑零改动；线上 `1.25.1` 与 `1.25.2` 客户端入口哈希同为 `index-DB-aVZyg.js`（本轮改动全在文档与判据链）。
- `v1.25.1` 已发布且有下游可见物 ⇒ 不重锚，改出 `v1.25.2`；参赛交付物按 `v1.25.2` 重建。

## [1.25.1] - 2026-09-26

第二十八轮的收尾补丁（文档与判据，无产品行为改动）。

### Fixed
- 🔴 **`docs/ARCHITECTURE.md` 六段链路图的三处行号已漂移**：本轮改 `engine.js`（加弃权判定）之后，
  图里仍写着 `validateDiagnosis engine.js:133`、`ruleDiagnosis engine.js:192`、`llmDiagnosis engine.js:212`，
  而真实位置是 173 / 232 / 252 —— **路径真、内容假**的凭据（既有判据只核"文件存在"，从不核"行号指向什么"）。
  同时把知识库范围从 `kb-001..055` 更正为 `kb-001..060`，并补上第⑦步「弃权/范围外判定 `answerability`」。

### Added
- **常驻判据：文档里的 `文件:行号` 引用必须真的指向它声称的符号**（`tests/docs_link_guard.mjs` 第 17/18 项）。
  按 basename 建源码索引解析裸文件名（**不是猜固定路径**——初版猜路径导致扫到 0 处，被本判据自带的零输入守卫抓住）；
  引用前后 60 字内出现的函数名必须出现在该行 ±3 行内，否则点名「行号已漂移」。
  变异实测：把 `engine.js:48` 改回旧值 30 ⇒ `FAIL … 第 30±3 行不含所声称的 extractState（行号已漂移）`，sha256 逐字节回滚。
- 同批补一条**输入面守卫**（扫到的行号引用 ≥3 处才算在射程内），防止正则失效时"零命中＝通过"。

### Notes
- 三条产品红线逻辑零改动；本补丁只影响文档与判据链。`v1.25.0` 已发布且有下游可见物（Release 四资产 + ghcr 镜像），
  故按既有纪律**不重锚**、改出 `v1.25.1`；参赛交付物按 `v1.25.1` 重建。

## [1.25.0] - 2026-09-26

第二十八轮。对标轴＝**「引擎知道自己不知道吗」——弃权 / 范围外第三态（台账 #52）**，
并把上一轮我自己立的两条前置锁（#70 地板零余量＝语料冻结、#71 逐用例回归锁）一并落地。
本轮**有对外行为变化**：新增 `abstain` / `scope_status` / `top_evidence_score` / `abstain_reason` 四个响应字段与一张弃权卡。

### Added
- **弃权第三态**（关 #52）：`rag.js` ↔ `rag.py` 同名同值新增 `ABSTAIN_T` 与 `answerability(evidence, flags)`；
  `engine.js` ↔ `engine.py` 在组装结论后判三态——证据不足 → `insufficient-information`，零证据 → `out-of-scope`，
  否则 `in-scope`。弃权时**只出一张「信息不足，建议补充问诊」卡、清空鉴别诊断，但 `flags` 与 `evidence` 照常返回**。
  **红旗命中一律不弃权**（借鉴实测：`kheireddinedev00/Medico` 弃权时红旗仍抬优先级并配具名测试
  `test_out_of_scope_still_honours_red_flags`；`dmustapha/triage-0` 用三条决定性体征"禁止弃权"）。
- **阈值不是手调的**：`ABSTAIN_T = 48.491` 由 `tests/ood_probe.mjs` 在当前语料 + 当前分词下取
  「域外最高 36.559」与「危急域内最低 60.422」的中点；probe 与常量**互相对账**（改语料/分词不重算即判红、
  双端不同值即判红、T 落到间隙之外即判红）。形状抄 `triage-0` 的 sane-band 守卫测试。
- **新守卫 `tests/abstain_guard.mjs`（二十一件套第 21 件，20 项）**：双端 T/文案逐字同值、13 条规则表派生探针走
  **真实链路**证明"红旗命中即不弃权"、纯函数边界三条（零证据/极低分带红旗/分数达标）、10 条域外输入双端逐条一致、
  弃权对外文案过**全站红线裸子串扫描**、复用 probe 的两个 GATE 把阈值锁接进阻断链。
- **契约与 UI**：`docs/openapi.json` Diagnosis 增四字段（含枚举与说明）、`backend/app/models.py` 的 `DxResult` 同步声明
  （漏这步 FastAPI 会把字段丢掉）；`Dx.jsx` 弃权卡 + `.banner.warn` 样式 + `SCOPE_LABEL` 三态可读措辞。
- **E2E 双向验收**（第七件）：真实降级链路**不出**弃权卡（反向对照，防常驻渲染假通过）；
  用合成响应注入 `abstain=true` 后浏览器**同时**渲染弃权卡与红旗横幅（先试过 `route.fetch()` 改写真实响应，
  在本运行时拿不到期望结构 ⇒ 改为整份合成，唯一变量就是 abstain）。
- **#70 地板余量可见化**：`retrieval_baseline.json` 新增 `measured` / `headroom` / `enforced` / `per_case` / `updated`；
  地板数值**一位未放宽**（沿用 55 条时代值），但"零余量"这件事从此被机器点名——`余量非零` 与 `headroom == measured − minimum`
  两条断言 + **对已提交版本比**的只升不降对账（手改低地板会被点名，浅克隆则显式 SKIPPED 不静默记绿）。
- **#71 逐用例检索回归锁**：50 例逐用例 R@5/MRR 快照入库，任一例退化即判红并点名
  （实测反例：抬高 `ret-21` 快照 → `逐用例退化（#71）：ret-21 mrr 0.55→0.25`）。破锁必须
  `--rebaseline-cases --why "<理由>"`，理由写进 fixture 并**每次运行公示**（visible debt）。
- `retriever_parity` 把 3 条**带标点查询**并入双端比对（53 例 × 3 档），并加 `PUNCT FILTER` 正反双向断言。
- `live_smoke` 自证从 17 项增至 **22 项**：弃权好数据对照、弃权仍给鉴别判红、`flags` 退化 null 判红、
  两态互斥判红、新字段被回退判红；线上另加一发真实域外请求（含"弃权不是错误、必须仍 200"）。

### Fixed
- 🔴 **主指标分母虚高（本轮实测自纠）**：`CRITICAL_RE` 用裸字 `/阻塞/` 本意是"视网膜中央动脉阻塞"，
  却把"慢性**阻塞**性肺疾病"也算成危急用例 ⇒ 危急数从 23 例被抬高。改为显式词表后
  **危急用例 23 → 17**，top-3 召回 22/23 → **17/17**；并新增"词表命中分布"打印（分母怎么来的必须可见）。
- 🔴 **我自己的新文案撞上自家红线判据**：弃权理由原写"…不替代医生判断"，而全站红线判据是**裸子串**
  `not.toContain("替代医生")` —— 含否定语义仍子串命中（与第二十四轮 `无气促` 命中 `气促` 同族第三次）。
  修法＝改文案为"最终判断由执业医生作出"，**不动判据**；并把"弃权文案必须过裸子串扫描"钉成常驻判据。
- 🔴 **一次差点没测到的判据强度边界（如实记）**：变异实验摘掉"红旗优先于阈值"分支后，
  **只有纯函数那条断言抓红，真实链路探针没抓**——因为现有红旗用例的分数本来就远高于阈值。
  结论：该守卫目前靠单元级检查撑着，链路级要等出现"低分危急用例"才有效；已记台账 #75 而不是宣称已覆盖。

### Measured
- 31 例合成病例自洽度：top-1 **28/31 = 90.3%**（与上轮同值）、top-3 **30/31 = 96.8%**、
  危急类 top-3 **17/17 = 100%（主指标，分母已修正）**、`kb_gap` 0、**弃权 3 例其中过度弃权 1 例**（`over_abstain_max = 1` 设地板）。
- 域外可分性（10 条域外 + 14 条危急域内）：间隙 **23.863**（上轮 18.770；标点修复后两侧同时拉开），
  域外判出 10/10、危急误伤 0/14、连带弃权域内 3 条（上轮 6 条）。
- 检索质量（silver 50 例，本轮未改语料）：R@5 0.860 / MRR 0.872333 / nDCG@5 0.802806 / 红旗子集 0.869565，
  对地板余量 0.01 / 0.005 / 0.0139 / 0.0217。
- 已知局限：阈值对**转录重复**敏感（同一句无关输入重复 6 次，top 分从 ~31 抬到 56.07 从而越过阈值，本轮实测）
  ⇒ 只主张"对单次域外输入弃权"，已立台账 #74。

### Notes
- 三条产品红线逻辑零改动；默认检索档仍 bm25；评测仍是同一批 31 例。对外新增面只有"会弃权"这一项。
- 本轮不引入外部依赖、不引入向量服务、不加任何可调阈值旋钮（`ABSTAIN_T` 是实算结果的记录，不是搜索出来的参数）。

## [1.24.0] - 2026-09-26

第二十七轮。对标轴＝**「可扩展性的真实代价：加一条知识要动几个文件、有没有判据兜住」**——本轮不靠读 peer 文档下结论，
而是把台账 #53（按 5 例 `kb_gap` 扩库）当作测量手段本身执行一遍，同时逐文件取证同类项目"规则/知识是数据还是代码"。

### Added
- **知识库 55 → 60 条**（关 #53）：新增 `kb-056` 胃食管反流病（GERD）／`kb-057` 肋软骨炎·胸壁疼痛／`kb-058` 社区获得性肺炎／
  `kb-059` 功能性消化不良／`kb-060` 高血压急症·重度高血压伴靶器官损害。5 条 `url` 全部落在 `kb_guard` 已核验域名白名单内
  且**当天实测 HTTP 200**（`www.cma.org.cn`／`www.medjournals.cn`；`www.nhc.gov.cn` 当天实测 412 故本次未用）。
  `SYMPTOM_TO_KB` 同轮补 13 个线索键（否则「无孤儿条目」判据当场拦停）。
- **`docs_link_guard` 第 13/14 项**：① 文档里的知识库条数改为**由 `KNOWLEDGE_BASE` 现算对账**（延续 #51 的派生化路线），
  上岗即点名 4 份文档 7 处过期「55 条」；② `EVAL_CARD` 第 4 节病种覆盖清单与知识库**逐条逐域全等比对**（条目改名、挪域、
  漏列都点名），变异实测把「社区获得性肺炎」改成「…X」即 `FAIL 库内有…但清单未列 | 清单有…但库内已无此条`，sha256 逐字节回滚。
- **`accuracy_guard` 新增 kb_gap 口径矛盾断言**：标了 `kb_gap` 的用例不得同时被判为 top-1 命中。变异实测 `FAIL …：ev-01`。

### Fixed
- 🔴 **标点与罕见单字一直进着 BM25 索引（本轮被扩库打穿才现形）**：`tokenize` 为"逐字 + 相邻二元组"，
  因此「，」以 **df=54** 成为检索词、口语句里的单字「来」（df=1）拿到全库最高 IDF——新增的 `肋软骨炎` 条目
  仅因正文含"来源"二字，就在肺栓塞用例上顶到第 1 名。扩库首跑四项检索指标同时跌破地板（R@5 0.85→0.83、
  MRR 0.867→0.850、nDCG@5 0.789→0.773、红旗子集 0.848→0.804），逐用例归因后才定位到这里。
  修法＝双端 `tokenize` 剔除 Unicode 标点/符号再切 n-gram（**不加任何可调参数，不是调阈值**）：
  修完 60 条库五项指标全部高于 55 条时代的地板，且**地板一位未放宽**。
  机制级说明与处置见 `docs/PITFALLS.md` H3；同时记 H4「排除性表述把对手的病名写进自己条目」也是同类污染。
- 🔴 **`floors.top1_min` 写了三轮但从没被读过**：fixture 里 `top1_min: 80` 自第二十四轮就在，而 `accuracy_guard` 只比
  `critical_top3_min`/`top3_min`——top-1 地板形同不存在。补接线并把三个键一起纳入形态校验，变异实测（抬到 99）
  `FAIL top-1 命中 90.3% < 地板 99%`。与 `ci_watch`"承诺在文档、实现在另一条路"同族。
- **一次自己差点写错的归因（如实记）**：扩库首跑 top-1 从 26 掉到 22，我先判为"新增条目挤掉既有正确诊断"，
  并用 `git stash` 把 55 条旧库跑同一批用例做对照——**ev-06/08/09 三例在旧库上输出完全相同**，即不是回归。
  真因是 `kb_gap` 用例此前可凭 `expect_other`（如 ACS）计入 top-1「命中」，补齐真正答案后反而不再命中该名单。
  已把该口径钉成断言（见 Added 第三条），并把 5 例 gold 从 `kb_gap` 翻成 `expect_top1`。
- **文档计数与实测指标同步**：`README.md`／`docs/ARCHITECTURE.md`／`docs/EVAL_CARD.md` 条数与第 4 节清单全部改为派生口径；
  `docs/ARCHITECTURE.md` 增写「病种域＝按 `scope` 首段去重」这一此前未定义的算法（旧值 19 与新值 20 都用它得出）。

### Measured
- 诊断排序（31 例合成病例自洽度，非真实世界准确率）：top-1 **26/31=83.9% → 28/31=90.3%**、top-3 **29/31=93.5% → 30/31=96.8%**、
  **危急类 top-3 召回 22/23=95.7% → 23/23=100%（主指标）**、`kb_gap` **5 → 0**。
- 检索质量（silver 50 例，60 条库 + 标点修复后）：R@1 0.547→**0.577**、R@3 0.793、R@5 0.850→**0.860**、
  MRR 0.867→**0.872**、nDCG@5 0.789→**0.803**、红旗子集 R@5 0.848→**0.870**；地板保持 55 条时代数值不动。；剩余唯一 top-3 未命中仍是 `ev-25`（域外输入，台账 #52）。
  地板按实测只升不降：`top1_min` 80→87、`top3_min` 90→93；`critical_top3_min` 维持 95（分母仅 23 例，不设零余量地板）。
- 语义邻接表随语料重算：`entries=60 pairs=480 avg_neighbors=8.0 empty=0 corpus_sha=890fed2b…`（构建期实算权重指纹，非手抄）。
- **可扩展性代价实测（我方，真实一次扩库的完整账）**：新增 5 条知识 = 改 **1 个权威源文件**（`knowledge.js`）+ 1 个 gold fixture
  + 2 次生成（`npm run kb:export`、`build_semantic_neighbors.py`）+ 4 份文档 7 处条数由判据点名 + **4 轮内容级消歧**
  （排除性病名污染、罕见单字碰撞）+ 1 个检索层机制缺陷修复 + 全套回归（二十件套／后端 6 套件／双端 53×3 比对）。
  结论：我方"加一条知识"**不是纯数据操作**，且地板零余量使语料事实上处于冻结状态——这给台账 #55（规则/知识 JSON 外置）
  提供了量化依据：外置本身不解决检索耦合，必须先修耦合再谈外置。
- **peer 侧同维度实测（本人 `gh api` 复跑，非转述）**：`kheireddinedev00/Medico` 全部临床阈值在
  **1 个 25,142 B 的 `data/triage_rules.json`**（10 顶层键含 `_note`/`_editing` 编辑指引），载入器 4 处 `raise RuleSetError`
  ＋ pydantic `extra="forbid"`，`tests/test_triage_rules.py` **17 个 test** 专测这份 JSON（含重叠带/留空/未知优先级/重复 id 全部拒载），
  红旗 22 条 + 否定线索 10 个 + 窗口 15 字符；`infiniflow/ragflow` `deepdoc/parser/` **18 个 `*_parser.py`**、
  `rag/svr/task_executor.py:112` `FACTORY` **16 键**、`ParserType` **15 员**、`SupportedLiteLLMProvider` **40 员**——扩展点是
  类/文件注册（改 `__init__.py` + `FACTORY` + enum 三处，另有 Go 侧孪生分发），不是数据文件；`openemr/openemr`
  `interface/modules/custom_modules/` **7 个自带模块**＋zend 模块 **11 个**，清单文件是 `info.txt` 自由文本（仓内根 `modules/` 实测 404）；
  `dmustapha/triage-0` 临床知识在 `src/triage/protocol-table.ts`（67,406 B／1,136 行，19 条协议 + 30 类枚举）**是代码不是数据**，
  载入零校验（校验只存在于测试期）；`bloodworks-io/phlox` 16 个工具在 `registry.py` 硬编码；另两家把规则外置成 JSON 但**零校验**
  （`desikas2007/patient-triage-assistant` 20 条规则×11 字段、文件缺失即静默空载；`Kabilan1710/AI-for-Symptom-Checker` 289 B 三键）。

### Notes
- 三条产品红线逻辑零改动、默认检索档仍 bm25、评测仍是同一批 31 例；本轮改动全部落在知识库数据、判据与文档。
- 🔴 **本轮自纠一处取证可追溯性缺陷**：第二十四轮报告的功能可比 peer 表只写了简称（"Medico"）没写 `owner/repo`，
  本轮想复用该结论时**无法定位到仓库**，只能靠 `gh api search/code` 反查（先误命中一个同名但结构不符的仓）。
  已把 owner/repo + 取证命令 + 日期写进本轮报告，并立台账 #68 要求此后所有对外引用的 peer 结论必须带可复现坐标。
- 发布纪律：v1.24.0 有下游可见物（Release 资产 + ghcr 镜像）才打 tag；tag 推送后等 run 落地再推下一次（台账 #63）。

## [1.23.2] - 2026-09-26

第二十六轮。**一条对外声称的溯源被证明"值是对的、但从不做机器校验"**，同时把上一轮记为"被外部产物阻塞"的
台账#53 追查到真因并解阻——真因不是"从来没这个文件"，而是**它在本机存在过，今天被我们自己的体量治理
当成可再生缓存清理掉了**。零对外行为改动。

### Fixed
- 🔴 **语义邻接表的权重指纹是手抄字面量**：`scripts/build_semantic_neighbors.py` 里
  `MODEL_SHA256 = "69a0b846…"` 是硬编码，构建时从不与**真正加载的权重文件**比对；而
  `semantic_guard` 的 provenance 检查只核 `/^[0-9a-f]{64}$/`（**形状**，不是来源）。
  ⇒ 换权重、甚至随手改这个串，产物头依然"溯源完整"。
  注意口径：该常量的**值本身是真的**（下文恢复的 `model.onnx` sha256 与之逐位相符），
  所以这不是假数据，是**真数据没有校验**——与 #34（源码包文件数手改常量）、#51（文档计数手抄）同族。
  改法：构建时对 `<engine-dir>/bge_onnx/onnx/model.onnx` 实算 sha256 填入；文件取不到即中止（不产出半成品表）。
- 🔴 **改造过程中我自己踩到的第二个坑（当场固化为断言）**：`HEADER` 原是**模块级 f-string**，
  求值早于 `main()` 给 `MODEL_SHA256` 赋值 ⇒ 第一次重建产出 `MODEL_SHA256=`（空串）的表。
  "把抄来的改成算出来的"如果只改赋值不改求值时机，结果是从"看起来可信"退化成"明显缺失"，
  而当时所有守卫和评测都还是绿的（空串只被 `/^[0-9a-f]{64}$/` 那条挡住，而它当时还没跑过）。
  修：`HEADER` 改成函数 `header()`，并加"长度必须为 64 否则中止"的断言——宁可不出表，也不出无指纹的表。
- **台账#53 的阻塞归因更正**：上一轮记作"生成器所需 ONNX 权重不随仓库发布 ⇒ 扩库做不了"。
  实测真因：`焚诀/eval/bge_onnx/onnx/model.onnx`（94,851,877B）**今天 14:56 被体量治理 L3 当"C 类可再生缓存"
  移入回收站**（`_trash/recycle_manifest.jsonl` 条目 `20260926-145600-b2174c`，同批 598 文件 / 触发理由
  「427,882,022B > 上限 52,428,800B」）。已按 sha256 核验后**复制恢复**（回收站原条目保留，未做任何删除）。
  ⇒ 结论修正为：**扩库不是不可行，而是被一次误分类的自动清理打断**；#53 解除阻塞。

### Added
- **两条常驻判据（`semantic_guard` 22→24 项）**：① 生成器源码内不得出现
  `MODEL_SHA256 = "<64hex>"` 手抄字面量；② 生成器必须真的自行计算（`compute_model_sha256` 与
  `hashlib.sha256` 在位、且含"找不到权重文件即中止"的分支）。
  变异实测：把字面量抄回去 → `FAIL 生成器里没有手抄的权重指纹字面量` rc=1，sha256 逐字节回滚。
- **蒸馏链可复现性实证**（本轮做扩库前的前置自证）：用恢复的权重在**未改动的 55 条库**上重跑，
  产物 `entries=55 pairs=440`、`corpus_sha=542207992540fa85…`（与提交值同），表体与已提交版本
  **逐字节相同**，唯一差异是 `generatedAt`（09-25→09-26）；文件体积也回到 9,146B/9,148B。
  这一次同时证明：权重身份正确、蒸馏是确定性的、以及实算 sha 与原手抄值一致。
- **fail-closed 实证**：把 `--engine-dir` 指向一个只有引擎、没有权重的目录 ⇒
  `FAIL: 找不到权重文件 …（权重被当缓存清理过？先恢复再重建）` rc=1，且**已提交的表未被改写**
  （中止发生在写盘之前）。提示语直接指向本轮的真实成因。

### Changed
- 版本 `1.23.1 → 1.23.2`；`semantic_neighbors.js`/`.py` 仅 `generatedAt` 随重建更新。

**三条产品红线逻辑零改动**；本轮零对外行为改动（客户端入口与响应体不变）。
本地实测：二十件套 exit0、`semantic_guard` 24/0、coverage/lint/ruff/mypy(35 文件 0 error)/pre-commit 12 钩全绿、
两组反向验（手抄回归被抓、缺权重即中止且不污染产物）。


## [1.23.1] - 2026-09-26

补丁版。修一个**观察器自身**的缺陷——它安全（不产假绿）但在最该生效的时刻失效，
且与它自己 docstring 的承诺直接矛盾。`v1.23.0` 已发布且镜像与 Release 均公开可拉（有下游可见物）
⇒ 按本仓既有规矩**不重锚**，改出补丁版。

### Fixed
- 🔴 `scripts/ci_watch.py` 把「还没有 run」当成「看完了」：轮询退出条件写成
  `if not pending or 超时: break`，而一行 run 都还没被 GitHub 建出来时 `rows == []` ⇒ `pending == []`
  ⇒ `not pending` 恒真 ⇒ **第一次查询就退出**。实测后果：v1.23.0 推送后三条 run 全部 `success`
  （`gh run list` 可证），本脚本却报 `UNKNOWN 未观察到任何 run`。方向安全（不产假绿），
  但工具恰好在「刚推完」这一它存在的核心场景失效，且会让人转回去手刷网页。
  根因归类＝本仓 B 节反复出现的**空集恒真**一族（"没扫到"被当成"扫过且干净"）。
  另注意：本文件 docstring 第 10 行本来就写着「run 创建有延迟……不在第一轮就下结论」，
  实现没做到——又是"承诺在文档、实现走另一条路"（与第二十二轮 `ERRORS.md` 4xx 日志那条同形）。

### Added
- **`python scripts/ci_watch.py --selftest`（离线自证，已挂进 CI 的 `infra-lint`）**：注入假 fetch，
  不打网络、不调 `gh`，五条路径——① 空集必须按 deadline 轮询 ≥5 次（旧实现在这条直接判红）
  ② 空集最终仍返回空集，让上层判 UNKNOWN 而不是 PASS ③ 第 3 次查询才出现的 run 要等到
  ④ 有 run 但仍 `in_progress` 时不得提前返回 ⑤ `--expect 3` 而只见 2 条时等满而不是按"已全绿"返回。
  轮询逻辑同时从 `main()` 抽成 `poll_runs()`，**main 内不留第二份实现**——否则"改了一份、
  另一份还带着老 bug"就是这条缺陷的复发路径。
- README Limitations 增第 4 条（原 4–7 顺延为 5–8）：**断网可用 ≠ 离线有 AI**。
  无密钥时零外呼是常驻判据（`live_path_guard.mjs` 断言 `calls.length === 0` 且 `mode === rule-fallback`，
  本轮复跑通过），五步链路断网仍出完整结果，但结论来自确定性规则模板；
  同类 `dmustapha/triage-0` 已做到设备内跑本地模型（离线仍有推理）。两者不等价，明确写开避免误读。
- `docs/PITFALLS.md` 新增 **B9**（症状→根因→处置→常驻判据齐全，判据即上面的 `--selftest`）。
- **CHANGELOG 小节单调性判据（台账#59，`version_guard` 11 项）**：上一个已发布 tag 里每个 `## [x.y.z]`
  今天必须仍在，并带"上一版本小节 ≥5 否则不许记绿"的分母自证。
  立论是本轮**我自己第四次**踩到"整节标题静默消失"：插入 1.23.1 时替换文本漏写回 `## [1.23.0]` 标题行，
  1.23.0 正文被并进 1.23.1 小节，而 `version_guard` 当时照样 9 pass / 0 fail —— 它只核"当前版本有没有小节"，
  对"历史小节丢了"全盲。append-only 是 CHANGELOG 的立命之本，判据却只核增不核删。
  变异实测：删掉 `## [1.23.0]` 标题行 → `FAIL ... 丢失=1.23.0` rc=1，按 sha256 逐字节回滚。

**判据生效的现场证据**：本轮写 1.23.2 小节时我**又一次**漏写回上一节标题（同一错误的第 5 次），
上一轮刚上线的「小节单调性」判据立刻 `FAIL … 丢失=1.23.1` rc=1 把它抓回。这类错不是记性问题，
而是「同一个值抄在 N 处、改的人不会记得逐处同步」的结构性漂移——所以它必须是判据而不是提醒。

### Changed
- 版本 `1.23.0 → 1.23.1`（四处声明同刻抬齐＋本小节；Release 正文仍由机器取自本小节）。

**三条产品红线逻辑零改动**；本轮零对外行为改动（仅工具链与文档），客户端入口与线上响应体不变。
本地实测：二十件套 exit0、`version_guard` 9→11 项、`ci_watch --selftest` 5/5、真 sha 观察 `[GATE:ci-watch-pass]`、
`--wait 0`「只看当前」路径不退化、ruff/mypy(35 文件 0 error)/ESLint 全 0、`docs_link_guard` 12 pass、
六条 infra 门禁全过、pre-commit 12 钩。
